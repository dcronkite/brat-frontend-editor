...
//                 };
                ...
                $.each(sourceData.highlightDict || [], function(num, dd) {
                    if (chunk.text.toLowerCase().search(dd.term) !== -1) {
                        svg.rect(highlightGroup,
                            chunk.textX - chunk.to + chunk.from,
                            chunk.row.textY + sizes.texts.y,
                            chunk.to - chunk.from + sizes.texts.widths[chunk.text],
                            sizes.texts.height,
                            {
                                fill: dd.color, //opacity:1,
            ...
            if (sentenceText) {
                // svg.text(textGroup, sentenceText); // avoids jQuerySVG bug
                // $.each(sentenceText._parts, function(num, arr) {
                //   svg.rect(
                //     highlightGroup,
                //     arr[2].x - 2,
                //     arr[2].y - sizes.fragments.height,
                //     5.5 * arr[1].length,
